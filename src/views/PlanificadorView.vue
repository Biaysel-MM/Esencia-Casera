          ...
          <div>
            ...
            <div v-else class="mb-6 grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
              <div v-for="day in weekDays" :key="day.date"
                class="rounded-2xl border border-[rgba(0,0,0,0.08)] bg-white p-5 shadow-[0_2px_8px_rgba(0,0,0,0.05)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_4px_16px_rgba(0,0,0,0.1)]">
                <div class="mb-4 flex items-center justify-between border-b border-[rgba(0,0,0,0.08)] pb-3">
                  ...
                  <button
                    class="flex h-9 w-9 cursor-pointer items-center justify-center rounded-xl border border-[rgba(0,0,0,0.08)] bg-[rgba(93,162,113,0.1)] text-[#5DA271] transition-all hover:border-[#5DA271] hover:bg-[rgba(93,162,113,0.2)]"
                    @click="generateDayMenu(day.date)" title="Generar menú para este día">
                    <span class="iconify h-4.5 w-4.5" data-icon="mdi:auto-fix"></span>
                  </button>
                </div>

                <div class="flex flex-col gap-3">
                  <div v-for="mealType in mealTypes" :key="`${day.date}-${mealType.key}`"
                    class="cursor-pointer rounded-xl border border-[rgba(0,0,0,0.08)] bg-[rgba(168,213,186,0.05)] p-3 transition-all hover:bg-[rgba(168,213,186,0.1)]"
                    ...
                    <div class="mb-3 flex items-center justify-between">
                      ...
                      <button
                        class="flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg border border-[rgba(0,0,0,0.08)] bg-white transition-all hover:border-[#5DA271] hover:bg-[#5DA271] hover:text-white"
                        @click.stop="openRecipeSelection(day.name, mealType.key)" title="Agregar comida">
                        <span class="iconify h-4 w-4" data-icon="mdi:plus"></span>
                      </button>
                    </div>

                    <div v-if="getMealForSlot(day.name, mealType.key)"
                      class="group relative cursor-pointer rounded-xl border border-[rgba(93,162,113,0.2)] bg-white p-3 transition-all hover:-translate-y-0.5 hover:border-[#5DA271] hover:shadow-[0_4px_12px_rgba(0,0,0,0.1)]"
                      @click.stop="openRecipeSelection(day.name, mealType.key)">
                      <button
                        class="absolute right-2 top-2 z-2 flex h-6 w-6 cursor-pointer items-center justify-center rounded-full bg-[#d4183d] text-white opacity-0 transition-opacity hover:shadow-[0_2px_4px_rgba(0,0,0,0.1)] group-hover:opacity-100"
                        @click.stop="removeMeal(day.name, mealType.key)" title="Eliminar">
                      ...
                      <div class="flex items-center gap-3">
                        <div class="h-12.5 w-12.5 shrink-0 overflow-hidden rounded-lg border border-[rgba(0,0,0,0.08)]">
                          <img :src="getMealForSlot(day.name, mealType.key)?.image_url || defaultImage"
                            :alt="getMealForSlot(day.name, mealType.key)?.title" class="h-full w-full object-cover"
                            @error="handleImageError">
                        </div>
                        <div class="min-w-0 flex-1">
                          <p class="truncate text-[13px] font-medium text-[#2C2C2C]">{{ getMealForSlot(day.name,
                            mealType.key)?.title || 'Sin asignar' }}</p>
                    ...
                    <div v-else
                      class="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-[rgba(0,0,0,0.08)] bg-[rgba(168,213,186,0.05)] py-6 text-[#6C7A6C] transition-all hover:border-[#5DA271] hover:bg-[rgba(168,213,186,0.1)] hover:text-[#5DA271]"
                      @click.stop="openRecipeSelection(day.name, mealType.key)">
            ...
            <div v-if="isSelectingRecipe"
              class="fixed inset-0 z-2000 flex items-center justify-center bg-black/50 p-5 backdrop-blur-sm"
              @click="closeRecipeSelection">
              <div
                class="flex max-h-[80vh] w-full max-w-200 flex-col overflow-hidden rounded-2xl border border-[rgba(0,0,0,0.08)] bg-white shadow-2xl"
                @click.stop>
                <div class="relative border-b border-[rgba(0,0,0,0.08)] bg-white p-6">
                  ...
                  <button
                    class="absolute right-5 top-5 flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg text-[#6C7A6C] transition-all hover:bg-[#D8EBD0]"
                    @click="closeRecipeSelection">
                ...
                <div class="flex-1 overflow-y-auto p-6">
                  <div class="mb-4">
                    <div class="relative">
                      <span class="iconify absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#6C7A6C]"
                  ...
                  <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div v-for="recipe in filteredRecipesForModal" :key="recipe.id"
                      class="cursor-pointer overflow-hidden rounded-xl border border-[rgba(0,0,0,0.08)] bg-white transition-all hover:-translate-y-0.5 hover:border-[#5DA271] hover:shadow-[0_8px_16px_rgba(0,0,0,0.1)]"
                      ...
                      <div class="relative h-30 overflow-hidden">
                        <img :src="recipe.image_url || defaultImage" :alt="recipe.title"
                          ...
                          @error="handleImageError">
                        <div
                          class="absolute left-2 top-2 rounded bg-[#5DA271] px-2 py-0.5 text-[10px] font-medium text-white">
                          {{ getCategoryLabel(recipe.category) }}</div>
                      </div>
                      <div class="p-3">
            ...
            <div v-if="showGenerateWeeklyModal"
              class="fixed inset-0 z-2000 flex items-center justify-center bg-black/50 p-4"
              @click="closeGenerateWeeklyModal">
              <div class="relative max-h-[90vh] w-full max-w-225 overflow-y-auto rounded-2xl bg-white p-8 shadow-2xl"
                @click.stop>
                <button
                ...
                <div class="mb-8">
                  ...
                  <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <label
                      class="flex cursor-pointer items-center gap-2 rounded-xl border border-[rgba(0,0,0,0.08)] p-3 transition-all hover:bg-[rgba(168,213,186,0.1)]">
                      ...
                      <span
                        ...
                        :class="{ 'border-[#5DA271] bg-[#5DA271]': generationPreferences.quick }">
                    ...
                    <label
                      ...
                      <span
                        class="flex h-5 w-5 items-center justify-center rounded-md border-2 border-[rgba(0,0,0,0.08)] transition-all"
                        :class="{ 'border-[#5DA271] bg-[#5DA271]': generationPreferences.healthy }">
                ...
                <div class="mb-8">
                  ...
                  <div v-if="generatedWeeklyMenu.length === 0"
                    class="flex flex-col items-center justify-center rounded-xl bg-[rgba(168,213,186,0.1)] p-12 text-center text-[#6C7A6C]">
                    <p>Selecciona preferencias y genera el menú</p>
                  </div>
                  <div v-else
                    class="grid max-h-75 grid-cols-1 gap-4 overflow-y-auto rounded-xl bg-[rgba(168,213,186,0.1)] p-4">
                    <div v-for="meal in generatedWeeklyMenu" :key="`${meal.day}-${meal.mealType}`"
                      class="overflow-hidden rounded-xl border border-[rgba(0,0,0,0.08)] bg-white">
                      <div
                        class="flex items-center justify-between border-b border-[rgba(0,0,0,0.08)] bg-[rgba(93,162,113,0.1)] px-4 py-3">
                        <h4 class="text-sm font-semibold text-[#5DA271]">{{ meal.day }}</h4>
                        <span class="rounded-full bg-[#5DA271] px-2 py-0.5 text-xs font-medium text-white">{{
                          getMealTypeLabel(meal.mealType) }}</span>
                      </div>
                      <div class="flex items-center gap-3 p-4">
                        <img :src="meal.recipe?.image_url || defaultImage" :alt="meal.recipe?.title"
                          class="h-15 w-15 rounded-lg object-cover" @error="handleImageError">
                        <div class="flex-1">
                ...
                <div class="flex gap-3 max-sm:flex-col">
                  <button
                    ...
                    @click="closeGenerateWeeklyModal">Cancelar</button>
                  <button
                    class="flex cursor-pointer items-center justify-center gap-2 rounded-xl bg-[#8b5cf6] px-6 py-3 font-medium text-white transition-all hover:bg-[#7c3aed]"
                    @click="generateWeekPreview">
                    <span class="iconify" data-icon="mdi:sparkles"></span>
                    Generar Vista Previa
                  </button>
                  <button
                    class="flex-1 cursor-pointer rounded-xl bg-[#5DA271] px-6 py-3 font-medium text-white transition-all hover:bg-[rgba(93,162,113,0.9)] disabled:opacity-50 disabled:cursor-not-allowed"
                    :disabled="generatedWeeklyMenu.length === 0" @click="applyGeneratedWeeklyMenu">
...
export default {
  ...
  setup() {
    ...
    const getWeekStart = (date = new Date()) => {
      ...
      // Si hoy es domingo (day === 0), restamos 6 días para ir al lunes
      // Si hoy es lunes (day === 1), restamos 0 días
      // Si hoy es martes (day === 2), restamos 1 día, etc.
    ...
    const generateWeekDays = (weekStart) => {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        // Asegurar que la fecha está en UTC para evitar desfases
